...
int main() {
	constexpr long long Base = MC_1_16, Seed = -1236314517, Trial = 32;
	...
	regex Feedback("#[0-9]+: (-?[0-9]+[.][0-9]+)  Mean: (-?[0-9]+[.][0-9]+)  SD: (-?[0-9]+[.][0-9]+)");
	regex Command("/tp (-?[0-9]+[.][0-9]+) (-?[0-9]+[.][0-9]+) (-?[0-9]+[.][0-9]+)");
	iTrace Instance; string Input = format("CAL {0}", Seed);
	smatch Value; string Output = Instance(Input);
	ofstream save("data.txt", ios::noreplace);
	Instance(format("VER {0}", mc2str(Base)));
	Instance(format("ERR {0} {1}", Emean, Evar));
	save << Instance("CHECK") << endl << Input << endl << Output << endl;
	for (double Index = 0; Index < Trial; Index++) {
