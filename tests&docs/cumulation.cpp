...
int main() {
	constexpr long long Base = MC_1_16, Rmin = 1200, Rmax = 2880, Smin = 1, Smax = 1E8;
	Generator Source; StrongholdIter Target;
	long long Progress = 0, Function[Rmax - Rmin] = {0};
	for (long long Seed = Smin; Seed <= Smax; Seed++) {
		...
		for (long long Index = Radius - Rmin; Index < Rmax - Rmin; Index++)
			Function[Index]++;
	}
	ofstream save("data.csv", ios::noreplace);
	for (long long Index = 0; Index < Rmax - Rmin; Index++)
		save << format("{0},{1}", Rmin + Index, Function[Index]) << endl;
}