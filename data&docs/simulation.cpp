...
public:
	...
	explicit Simulator(long long Base, long long Seed) {
	...
	string solve(double Emean, double Evar, bool Legacy) {
		...
		else return format("/execute in minecraft:overworld run tp @s {0:.2f} 240.00 {1:.2f} {2:.2f} -32.00", PosX, PosZ, Yaw);
	}
	...
	string calib(double Emean, double Evar, bool Legacy) {
		...
		if (Legacy) {
			...
			double Error = remainder(Yaw - Angle, 360);
			Esum2 += Error * Error, Count++;
			return format("/execute in minecraft:overworld run tp @s {0:.2f} 240.00 {1:.2f} {2:.2f} -32.00", PosX, PosZ, Yaw);
		}
	}

	double NBcalib() const { return sqrt(fmax(0, Esum2 / (Count - 1))); }

};

int main() {
	constexpr long long Base = MC_1_16, Seed = -1236314517, Trial = 30;
